...
// experiment with using functors for twiddling bits in
// a GPIO register. I haven't noticed anyone doing this and
// I was just interested in seeing how this approach would
// work out.
//
// Normally this would just mean abstracting such code into
// one or more functions and creating some kind of readable
...
// "raw bits" technique.  That said, I've always considered
// bit-fields to be a more natural looking way to
...
//
//      1)  functors for a GPIO register that is part of a
//          custom design (e.g., a custom controller board);  and
...
//
//  The irony of using a template to operate on a GPIO register hardwired to
// a custom design can be summed up in two points:
//
//      1)  the point of templates is to facilitate generic programming;
//      2)  I cannot conceive of a way to abstract bitfield usage in such
//          a way that one template can accomodate all designs for using
//          a GPIO register.
//
//  Put another way, the highly hardware implementation specific
//  concerns of register bit-twiddling defeats generic programming's
//  intent of code reuse.  Consequently, the template code for something
//  like a GPIO register will be a one-off, unreusable code.
//
//  Unresusable code means that you're not much better off than writing
//  bit manipulation functions in C as compared to templates, at least
//  with respect to the number of lines of code.
//
//  there is no wa GPIO register hardwired to a particular design
...
//  customizeable registers on other, unreleated, designs. Of
//  course this code might be reusable should the design team
//  reuse the hardware design pertaining to the GPIO register, say
//  on another version of the control board.
//
//
//
...
//           say, the lamp ID. Some kind of accomodation for passing an
